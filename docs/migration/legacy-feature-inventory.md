...
| 增高短语：`;fzg` | `legacy/string_change.ahk:15-22`；固定短语 + `InsertRedText()` + 50 ms + Left 4 | `src/hotstrings.ahk`；相同固定短语 + transactional CF_HTML/Color Reset + 50 ms caret-settle + Left 4 | 部分迁移：文本和光标语义一致；富文本来源、颜色复位和成功条件不同，optimized production path 待 Windows 复测 | 高 | 同名热字符串直接冲突 | 保留新实现作为目标；Windows 验证红字、连续输入、clipboard 和光标位置后停用 legacy duplicate | v0.5.0 |
