...
| 增高短语：`;fzg` | `legacy/string_change.ahk:15-22`；固定短语 + `InsertRedText()` + 50 ms + Left 4；不执行颜色复位 | `src/hotstrings.ahk` → phrase-specific transactional CF_HTML + 50 ms + Left 4；返回 `COLOR_RESET_NOT_REQUIRED` | caret-order A/B 与最终 generated release 均已验证；保留 current CF_HTML、safe clipboard timing 和 `Left 4` | 高 | 同名热字符串直接冲突 | 新实现作为 mainline；internal-alpha coexistence 时停用 legacy duplicate | v0.5.0 |
...
| RAlt+H/J/K/L 方向移动 | `legacy/karabiner.ahk:3-6` | `src/global_hjkl_arrows.ahk` | 已实现，默认关闭，待 Windows 验证 | 高；已确认是全局 HHKB navigation | `GlobalHjklArrows=true` 时与 legacy/compatibility 同键冲突；全局生效 | 完成 generated-release 实测后由新项目接管；不加 MedEx `#HotIf` | v0.5.0 M2 |
...
当前新项目默认启用的 hotkeys `Ctrl+Alt+Esc` 和 `Ctrl+Alt+Q` 与 `legacy/karabiner.ahk` 中的 active hotkeys 没有字面冲突。可选的 `GlobalHjklArrows` 启用后会与 legacy/compatibility 的 RAlt+H/J/K/L 冲突，不得双开。
...
- Legacy `;fzg` 固定等待 50 ms 后 Left 4；新项目使用 phrase-specific paste/restore → 50 ms → Left 4，不运行 Color Reset，并只在 paste/restore 成功后移动 caret。
...
新项目已覆盖 5 个报告文本入口；`;red`、`;fwj`、`;fjd` 使用已提升为 mainline 的 Candidate G，`;fzg` 使用已现场验证的 phrase-specific no-reset path，`;cmx` 保持简单 caret relocation。报告入口仍需要 MedEx-only entry guard 和 phrase-specific regression，不能与 legacy duplicate 同时启用。RAlt+H/J/K/L 已作为默认关闭的 `GlobalHjklArrows` 实现，待 Windows generated-release 验证；其他 active `legacy/karabiner.ahk` 阅片/键盘行为仍未迁移。
