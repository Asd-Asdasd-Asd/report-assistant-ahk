...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 红色 `（见图）` 插入：`;red` | `legacy/string_change.ahk:4-12` → `InsertRedText()` → `red_not.clip`；随后点击固定坐标恢复黑色 | `src/hotstrings.ahk:1-4` → `InsertRedFigureTextAndRestoreState()` → dynamic `CF_HTML` → `relativeMousePixelValidated` | 已迁移并完成 2026-07-16 generated-release Windows mainline validation；当前仅支持已记录 profile | 高 | 两个脚本注册同名热字符串，会重复触发或争抢触发；两种剪贴板路径也可能重叠 | internal-alpha coexistence 时禁用 legacy duplicate，只启用新实现；`uiaInvoke` 仅作显式 rollback | v0.5.0 |
| 增高短语：`;fzg` | `legacy/string_change.ahk:15-22`；固定短语 + `InsertRedText()` + 50 ms + Left 4；不执行颜色复位 | `src/hotstrings.ahk` → Schema 2 builtin template → `ReportTemplatePlan`；由 `{{cursor}}` 派生等价 caret relocation，返回 `COLOR_RESET_NOT_REQUIRED` | caret-order A/B 与最终 generated release 均已验证；active config 不保存固定 Left count | 高 | 同名热字符串直接冲突 | 新实现作为 mainline；internal-alpha coexistence 时停用 legacy duplicate | v0.5.0 |
| 未见明显增高短语：`;fwj` | `legacy/string_change.ahk:25-31` | `src/hotstrings.ahk:13-17` → shared CF_HTML/Candidate G orchestration | 当前代码已迁移；Candidate G mainline 已现场验证，仍需 phrase-specific regression | 高 | 同名热字符串直接冲突；当前新实现仍为 global | 共存时停用 legacy duplicate；下一检查点加入 MedEx-only entry guard | v0.5.0 |
| 摄取降低短语：`;fjd` | `legacy/string_change.ahk:34-40` | `src/hotstrings.ahk:19-23` → shared CF_HTML/Candidate G orchestration | 当前代码已迁移；Candidate G mainline 已现场验证，仍需 phrase-specific regression | 高 | 同名热字符串直接冲突；当前新实现仍为 global | 共存时停用 legacy duplicate；下一检查点加入 MedEx-only entry guard | v0.5.0 |
| 尺寸模板：`;cmx` | `legacy/string_change.ahk:43-48`；`cm×cm` + Left 2 | Schema 2 builtin `cm×{{cursor}}cm`；`ReportTemplatePlan` 派生 caret relocation | 已迁移（视觉行为等价；Windows regression 已覆盖） | 高 | 同名热字符串直接冲突 | 共存时禁用 legacy 版本，只保留新实现 | v0.5.0 |
...
| RAlt+H/J/K/L 方向移动 | `legacy/karabiner.ahk:3-6` | `src/global_hjkl_arrows.ahk` | 已实现；compatibility duplicate 已移除 | 高；已确认是全局 HHKB navigation | 与原始 `karabiner.ahk` 同键冲突；compatibility 已不再注册 | 共存时设置 `GlobalHjklArrows=true`，只由新项目接管；不加 MedEx `#HotIf` | v0.5.0 M2 |
...
当前新项目默认启用的 hotkeys `Ctrl+Alt+Esc` 和 `Ctrl+Alt+Q` 与 compatibility 中的 active hotkeys 没有字面冲突。Compatibility 已移除 RAlt+H/J/K/L；共存时可由新项目的 `GlobalHjklArrows=true` 单独注册。原始 `karabiner.ahk` 仍包含相同按键，不得与该配置同时运行。
...
- Legacy `;fzg` 固定等待 50 ms 后 Left 4；新项目由 `{{cursor}}` 和 rendered text 派生等价移动量，不运行 Color Reset，并只在插入成功后移动 caret。
...
- 当前 compatibility layer 的 Shift+Alt+S 只保证单次触发。持续按住 Alt+Shift 后连续按 S 时，第一次流程中的模拟输入可能释放逻辑修饰键，后续 S 可能进入当前输入框。该限制不影响单次使用，本轮不修改 keyboard hook、modifier restoration、SendInput 或 reentry；待 legacy compatibility layer 重构时统一处理。
