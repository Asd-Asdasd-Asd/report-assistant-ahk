...
解析、不存在跨 layout 共同点，或共同点无法映射到实际窗口，但 native tool
group 仍能唯一绑定 Viewer，resolver 会复用已采集的同 PID/root-owner
可见 image surface，在其 client 内部以避开常见分割线的 4×4 候选网格
只读执行 `WindowFromPoint`，要求命中
