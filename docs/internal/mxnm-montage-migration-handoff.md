...
R4C4 在 2026-08-03 布局 control 字段测试中的位置为 `xRatio=0.881579`、
`yRatio=0.771014`。2026-08-18 的另一现场反馈显示，同样配置为第 4 行第 4 列时，
该点实际选中了第 5 行第 4 列。因此这个旧点不再视为跨环境有效；需用 hover-only
布局五点诊断重新取得该环境的首格、行列步距和实际 R4C4 点后再改 production。
不要直接假设 `(column-0.5)/4`、`(row-0.5)/5`，也不要仅凭“偏移一行”盲减固定值。
