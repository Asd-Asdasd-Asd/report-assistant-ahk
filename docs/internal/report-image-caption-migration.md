...
Legacy `Shift+Alt+S` 在报告图像窗口内完成一个高频循环：

1. 复制当前选中的报告文字；
2. 点击下方图像描述输入区域并粘贴；
3. 将鼠标移到上方图像区域并发送一次 `WheelDown`；
4. 恢复原鼠标位置。
...
1. **Target window guard**：从触发时前台窗口确认进程、class、PID、root owner、
   client rect、DPI 和 monitor；全过程要求原 HWND 保持前台。
2. **Source selection capture**：只在已验证目标窗口发送一次 `Ctrl+C`，要求
   clipboard sequence/内容确实更新；使用共享 clipboard transaction，最终恢复
   用户原 clipboard。
3. **Caption target resolver**：优先使用唯一、visible/enabled、矩形合理的
   `Edit`/TextEdit/Value element；若供应商只暴露自绘区域，则使用固定静态锚点
   与当前窗口 client rect 推导区域内点，不使用屏幕绝对坐标。
...
- target window、PID 或 foreground 改变：立即停止；
...
诊断同时收集目标窗口/monitor/DPI、两个 point 的 native HWND parent chain、
UIA smallest-element parent chain、结构候选及精确静态锚点
