...
反馈：一张带三条文字线的深灰色卡片从 source 选区附近（鼠标不在 source 时退回
source client 中心）沿二次曲线缓入缓出，接近 caption point 时缩小并淡出。
caption 输入框缺少稳定的 UIA 边界，因此不绘制终点区域高亮。overlay 使用 token 取代旧动画，
不进入 clipboard、保存、翻页或结果判断逻辑；其含义仅为“保存点击已经派发”，
不声称后端保存成功。

## 2026-09-06 首次等待状态修正

首次保存前的 500 ms 等待由 Caption 业务的 target PID/HWND session 管理，普通路径仍为 20 ms。只有实际执行保存点击才标记该 target 已经过首次等待；解析目标、记录诊断或粘贴前取消不会消耗标记。同进程新 HWND 重新走首次等待。这里记录的是已派发的保存步骤，不证明后端持久化成功。200/550 ms 保存后等待与翻页间隔策略保持原逻辑。此次修改尚待 Windows 验收。
