...
已批准的 V1 在 paste transaction 完成后，通过 MedEx-specific adapter 使用 UIA anchors、比例坐标定位 color trigger，再对颜色菜单中 Name 为 `000000` 的 `Hyperlink` 调用 `Invoke()`。该方案是 Technical Investigation 结论，不是永久架构；详见 `docs/technical-investigations/2026-07-medex-rich-text-color-reset.md`。
...
MedEx color reset 返回 stable result code 和诊断 context，而不是单个 Boolean。至少覆盖：
...
调用层决定如何提示用户和是否继续 workflow；adapter 在所有不确定状态下 fail-closed，绝不继续 blind clicks。
