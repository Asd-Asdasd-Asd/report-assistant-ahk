...
已通过现场验证的 V1 在 paste transaction 完成后，从 foreground MedEx window root（或经现场证据确认的报告区域父容器）枚举 `Text` elements。它以 exact Name=`检查所见` 唯一定位目标语义行，再选择该行右侧、垂直对齐且 Name 匹配动态字号 pattern 的唯一 local anchor。颜色 trigger 使用该字号矩形与集中配置的 `ColorArrowOffsetX/Y` 定位；Name=`rAI` 仅作为可选 layout fingerprint，不参与成功条件。菜单打开后，对 exact Name=`000000` 的 `Hyperlink` 调用 `Invoke()`。V1 不假设 toolbar anchors 是 focused `Document` 的 descendants，也不要求整个 UIA root 落在 client area 内。该方案是可替换的 Technical Investigation 结论，不是永久架构。
