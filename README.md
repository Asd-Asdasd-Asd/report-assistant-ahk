...
Step 2 基线为 `7a0d9a2`；`2369b68`（tag `v0.6.0-candidate-g`）是 Candidate G promotion 基线。当前 Step 3 implementation 已通过 Windows success/fast-failure 验收。`relativeMousePixelValidated` 已是 production default：UIA 精确定位 `Name="检查所见"`，经过 profile geometry 校验后以相对坐标打开颜色菜单，四点 popup signature 匹配后最多单击一次黑色。`uiaInvoke` 仅保留为显式 comparison/rollback；两种策略之间没有 automatic fallback。
