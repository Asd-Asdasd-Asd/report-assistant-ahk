...
- 设置窗口可配置箭头、长度测量、3D SUV 和截图快捷键，支持独立勾选 Win 修饰键；四项默认关闭。
...
快捷键页提供箭头、长度测量、3D SUV 和截图。默认组合为 `Ctrl+Alt+1/2/3/4`，首次升级保持停用；Win 键通过输入框旁的独立选项加入。前三项在报告程序或 Viewer 前台时可用，并已完成当前工作站 Windows 非临床验证；3D SUV 会等待完整组合键松开后单次选择，之后不需要继续按住。截图只在 Viewer 前台时把所配按键映射为 MedEx F12，派发后让整个 Viewer 快速白闪约 90 ms，不声称截图已经完成。工具选择不移动鼠标或切换前台窗口；程序从 Vendor 配置计算按钮位置，并在点击前核对真实控件 ID，配置缺失、布局不符或 ID 冲突时拒绝执行。实现与现场发现见 `docs/internal/mxnm-viewer-tool-hotkeys.md`。
